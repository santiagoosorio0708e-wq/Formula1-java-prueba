...
import com.f1.simulacion.MotorSimulacion;
import com.f1.simulacion.SimulacionListener;
...
public class PanelSimulacion extends JPanel implements SimulacionListener {
    ...
    private void initComponents() {
        ...
        lblCircuito = createInfoLabel(infoPanel, "🛣️ Circuito", "Sin seleccionar");
        lblVuelta = createInfoLabel(infoPanel, "🔄 Vuelta", "0 / 0");
        lblClima = createInfoLabel(infoPanel, "🌤️ Clima", "---");
        ...
        btnIniciar = new F1Button("▶️ Iniciar", F1Button.Style.SUCCESS);
        ...
        btnPausar = new F1Button("⏸️ Pausar", F1Button.Style.SECONDARY);
        btnPausar.addActionListener(e -> {
            if (motor != null && motor.isEnCurso()) {
                motor.togglePausa();
                btnPausar.setText(motor.isPausado() ? "▶️ Reanudar" : "⏸️ Pausar");
                lblEstado.setText(motor.isPausado() ? "⏸️ PAUSADO" : "🟢 EN CURSO");
            }
        });
        controlPanel.add(btnPausar);

        btnDetener = new F1Button("⏹️ Detener", F1Button.Style.DANGER);
        ...
        JLabel alertTitle = new JLabel("📢 Alertas de Carrera");
    ...
    public void iniciarSimulacion(Circuito circuito, ConfiguracionVehiculo config) {
        ...
        agregarAlerta("🏁 Carrera configurada en " + circuito.getNombre() +
    ...
    public void onPitStop(EstadoCarrera estado) {
        agregarAlerta("🔧 PIT STOP: " + estado.getPiloto().getNombre() +
    ...
    public void onCarreraFinalizada(List<ResultadoClasificacion> resultados) {
        ...
        btnIniciar.setEnabled(true);
        agregarAlerta("🏁 ¡CARRERA FINALIZADA! Ganador: " +
    ...
    public void onCambioClima(CondicionClimatica nuevoClima) {
        ...
        agregarAlerta("🌤️ Cambio de clima: " + nuevoClima.getIcono() + " " + nuevoClima.getNombre() +
                " (Factor velocidad: x" + String.format("%.2f", nuevoClima.getFactorVelocidad()) + ")");
    }

    @Override
    public void onRetiro(EstadoCarrera estado, String motivo) {
        agregarAlerta("❌ RETIRO: " + estado.getPiloto().getNombre() + " - " + motivo);
    }
